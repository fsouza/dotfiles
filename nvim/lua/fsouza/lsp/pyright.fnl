...
(fn set-from-cmd [exec args cb]
  (let [cmd (require :fsouza.lib.cmd)]
    (cmd.run exec {:args args} nil #(if (= $1.exit-status 0)
                                      (cb (vim.trim $1.stdout))
                                      (cb nil)))))

(fn set-from-poetry [cb]
  (vim.loop.fs_stat "poetry.lock" #(if $1
                                     (cb nil)
                                     (set-from-cmd "poetry" ["env" "info" "-p"] cb))))

(fn set-from-pipenv [cb]
  (vim.loop.fs_stat "Pipfile.lock" #(if $1
                                      (cb nil)
      ...
      (let [folder (. folders idx)]
        (if folder
          (let [venv-candidate (path.join (vim.loop.cwd) folder)]
            (vim.loop.fs_stat (path.join venv-candidate "bin" "python") #(if (and (not $1) (= $2.type "file"))
                                                                           (cb venv-candidate)
                                                                           (test-folder (+ idx 1)))))
          (cb nil))))
