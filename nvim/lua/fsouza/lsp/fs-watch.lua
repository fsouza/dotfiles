...
local function make_fs_event_handler(root_dir, notify_server)
  ...
  local function notify(client_id, reg_id, filepath, events, kind)
    ...
    if events.rename then
      vim.uv.fs_stat(filepath, function(err, _)
        if err then
          try_notify_server(client_id, reg_id, uri, file_change_type.Deleted, watch_kind.Delete)
        else
          vim.schedule(function()
            if is_file_open(filepath) then
              try_notify_server(client_id, reg_id, uri, file_change_type.Changed, watch_kind.Change)
            else
              try_notify_server(client_id, reg_id, uri, file_change_type.Created, watch_kind.Create)
            end
          end)
        end
      end)
  ...
  return function(err, filename, events)
    if
      not err
      and not vim.endswith(filename, backupext)
      and not vim.startswith(filename, ".git/")
...
local function make_event(root_dir, notify_server)
  ...
  local ok, err =
    vim.uv.fs_event_start(event, root_dir, { recursive = true }, make_fs_event_handler(root_dir, notify_server))
