...
local function register_method(name, client, bufnr)
  ...
  local function handle_mappings(mappings)
    for _, mapping in ipairs(mappings) do
      vim.keymap.set(mapping.mode, mapping.lhs, mapping.rhs, { silent = true, buffer = bufnr })
    end
  end
...
local function lsp_attach(opts)
  ...
  local mappings = {
    ...
    { lhs = "<leader>dd", rhs = fuzzy.lsp_workspace_diagnostics },
    {
      lhs = "<c-n>",
      rhs = function()
        diag_jump(vim.diagnostic.goto_next)
      end,
    },
    {
      lhs = "<c-p>",
      rhs = function()
        diag_jump(vim.diagnostic.goto_prev)
      end,
    },
  }
  ...
  for _, mapping in ipairs(mappings) do
    vim.keymap.set("n", mapping.lhs, mapping.rhs, { silent = true, buffer = bufnr })
  end
end
