...
local function fmt(bufnr, client, callback)
  ...
  if client then
    local _, req_id = client:request(
      ...
      formatting_params(bufnr),
      callback,
      bufnr
    )
    
    return req_id, function() client:cancel_request(req_id) end
  end
end
...
local function autofmt_and_write(bufnr, client_id)
  local function do_autocmd()
    vim.api.nvim_exec_autocmds(
      {"User"},
      {pattern = "fsouza-LSP-autoformatted", data = {bufnr = bufnr}}
    )
  end
      ...
      pcall(function()
        ...
        fmt(bufnr, client, function(_, result)
          if vim.api.nvim_buf_is_valid(bufnr) and
             changed_tick == vim.api.nvim_buf_get_changedtick(bufnr) then
              ...
              vim.api.nvim_buf_call(bufnr, function()
                ...
                helpers.rewrite_wrap(function()
                  vim.lsp.util.apply_text_edits(
                    result, bufnr, client.offset_encoding
                  )
                end)
...
local function attach(bufnr, client_id, priority)
  ...
  if client and 
     not should_skip_buffer(bufnr) and 
    ...
    augroup(augroup_name(bufnr), {
      {
        ...
        callback = function() autofmt_and_write(bufnr, client_id) end
      }
    })
