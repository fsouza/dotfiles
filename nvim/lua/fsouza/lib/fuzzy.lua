...
local function file_actions()
  local actions = require("fzf-lua.actions")
  return {
    enter = function(selected, opts) edit_or_qf(function(sel, o) edit("edit", sel, o) end, selected, opts) end,
    ["ctrl-s"] = function(selected, opts) edit_or_qf(function(sel, o) edit("split", sel, o) end, selected, opts) end,
    ["ctrl-x"] = function(selected, opts) edit_or_qf(function(sel, o) edit("split", sel, o) end, selected, opts) end,
    ["ctrl-v"] = function(selected, opts) edit_or_qf(function(sel, o) edit("vsplit", sel, o) end, selected, opts) end,
    ["ctrl-t"] = function(selected, opts) edit_or_qf(function(sel, o) edit("tabedit", sel, o) end, selected, opts) end,
...
local function lsp_actions()
  ...
  actions.enter = function(selected, opts) edit_or_qf(save_stack_and_edit, selected, opts) end
  return actions
end
...
local function send_lsp_items(items, title)
  ...
  local opts = config.normalize_opts(
    {winopts = {title = title}, cwd = virtual_cwd},
    config.globals.lsp
  )
  ...
  for _, item in ipairs(items) do
    ...
    local formatted_item = make_entry.lcol(
      item,
      {
        cwd = virtual_cwd,
...
local function send_items(items_or_fzf_cb, title, opts)
  ...
  local actions = cb and {enter = cb} or 
                 (use_lsp_actions and lsp_actions() or file_actions())
  ...
  local function send_to_fzf()
    ...
    local fzf_opts = config.normalize_opts(
      {winopts = {title = title}, actions = actions},
      config.globals.lsp
    )
  ...
  local opts = config.normalize_opts(
    {
      ...
      cwd = cwd,
      actions = {
        enter = function(sel) handle_repo(run_fzf, cd, sel) end
      }
    },
...
local function pick_cwd()
  ...
  local opts = config.normalize_opts(
    {
      actions = {
        enter = function(sel) set_virtual_cwd_impl(sel[1]) end
      }
    },
    config.globals.files
  )
...
local rg_opts = "--column -n --hidden --no-heading --color=always --colors 'match:fg:0x99,0x00,0x00' --colors line:none --colors path:none --colors column:none -S --glob '!.git' --glob '!.hg' --glob '!*.ipynb'"

local mod = {
  files = files,
  git_files = git_files,
  live_grep = function(...) return live_grep(rg_opts, ...) end,
  grep = function(...) return grep(rg_opts, ...) end,
  grep_last = function(...) return grep_last(rg_opts, ...) end,
  grep_visual = function(...) return grep_visual(rg_opts, ...) end,
